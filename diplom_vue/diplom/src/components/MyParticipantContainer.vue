    ...
    export default {
        ...
        methods: {
            ...
            deleteUserFromTeam(user) {
                if(user.user_id != Number(localStorage.getItem('userId'))) {
                var curr2 = localStorage.getItem('currTeamId')
...
<template>
    <div class="manage-team">
            ...
            <div class="all-users">
                <input class="userAddEmail" placeholder="Введите почту участника..." v-model="userAddEmail">
                <button @click="addUserForTeam">Добавить участника</button>
                <div class="owners">
                    <div class="owners-title">Администраторы</div>
                    <div class="owner" v-for="admin in admins" :key="admin.id">
                        <div class="img-owner">
                            <img src="" alt="IMAGE">
                        </div>
                        <div class="name-owner">{{ admin.username }}</div>
                        <button @click="deleteUserFromTeam(admin)">Удалить</button>
                        <button @click="changeUserRoleInTeam(admin)">Поменять роль</button>
                    </div>
                </div>
                <div class="users">
                    ...
                    <div class="user" v-for="user in users" :key="user.id">
                        ...
                        <button @click="deleteUserFromTeam(user)">Удалить</button>
                        <button @click="changeUserRoleInTeam(user)">Поменять роль</button>
                    </div>
                </div>
                </div>
            </div>
